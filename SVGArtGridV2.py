#!/usr/bin/env python3
"""
SVGArtGridV2.py - A ComfyUI node that generates artistic SVG grids with image input
Based on SVGArtGrid.py with enhanced functionality for color extraction and composition
"""
...
class SVGArtGridV2:
    """A ComfyUI node that generates Kandinsky-style artistic SVG grids with image input support"""
    ...
    def _extract_palette_from_image(self, pil_image, color_count=5):
        ...
        if not SKLEARN_AVAILABLE:
            print("SVGArtGridV2: sklearn not available, using default palette")
            return [
                '#69D2E7', '#A7DBD8', '#E0E4CC', '#F38630', '#FA6900'
            ][:color_count]
    ...
    def _generate_composition_block(self, dwg, i, j, square_size, pil_image, block_styles, blend_factor=0.7):
        """Generate a block based on image composition."""
        x_pos = i * square_size
        y_pos = j * square_size
    ...
    def _generate_composition_grid(self, dwg, pil_image, num_rows, num_cols, square_size, block_styles, blend_factor=0.7):
        """Generate a grid based on image composition."""
        for i in range(num_rows):
            for j in range(num_cols):
                self._generate_composition_block(dwg, i, j, square_size, pil_image, block_styles, blend_factor)
    ...
    def _svg_to_png(self, svg_string, width, height):
        ...
        if CAIRO_AVAILABLE:
            try:
                ...
                print(f"SVGArtGridV2: cairosvg conversion failed: {e}")
        
        # Fallback to simple PIL representation
        print("SVGArtGridV2: Using PIL fallback for SVG rendering (cairo not available)")
    ...
    def generate_art_grid(self, image, rows, cols, seed, palette_index, mode, color_count, blend_factor,
        ...
        if square_size <= 0:
            print(f"SVGArtGridV2: Warning - Invalid square size {square_size}. Resetting to default.")
